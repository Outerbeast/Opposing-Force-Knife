...
/*
In your map's CFG use these commands:

as_command weapon_knife_damage 17 : Damage dealt by Primary attack of the knife (Entity's keyvalue "dmg" override this value)

as_command weapon_knife_damage_sub 10 : Damage dealt by Primary attack subsequents of the knife (Entity's keyvalue "fuser1" override this value)

as_command weapon_knife_damage_charg 30 : Damage dealt by Secondary attack of the knife (Entity's keyvalue "fuser2" override this value)

as_command weapon_knife_damage_throw 30 : Damage dealt by Tertiaryattack of the knife (Entity's keyvalue "fuser3" override this value)
...
class weapon_knife : ScriptBasePlayerWeaponEntity
{
	...
	{
		if( self.pev.dmg <= 0 )
			self.pev.dmg = float( weapon_knife_damage.GetInt() );
		if( self.pev.fuser1 <= 0 )
			self.pev.fuser1 = float( weapon_knife_damage_sub.GetInt() );
		if( self.pev.fuser2 <= 0 )
			self.pev.fuser2 = float( weapon_knife_damage_charg.GetInt() );
		if( self.pev.fuser3 <= 0 )
			self.pev.fuser3 = float( weapon_knife_damage_throw.GetInt() );
	...
	bool Swing( int fFirst )
	{
		...
		else
		{
			...
			if ( self.m_flNextPrimaryAttack + 1 < g_Engine.time )
			{
				...
				pEntity.TraceAttack( m_pPlayer.pev, self.pev.dmg, g_Engine.v_forward, tr, DMG_CLUB );  
			}
			else
			{
	...
	void TertiaryAttack()
	{
		...
		if( pTKnife !is null )
		{
			...
			pTKnife.pev.fuser3 = self.pev.fuser3;
...
CCVar@ weapon_knife_damage, weapon_knife_damage_sub, weapon_knife_damage_charg, weapon_knife_damage_throw;

void RegisterKnife()
{
	@weapon_knife_damage = CCVar("weapon_knife_damage", 17, "Knife damage on first Primary Attack", ConCommandFlag::AdminOnly);
	@weapon_knife_damage_sub = CCVar("weapon_knife_damage_sub", 10, "Knife damage on subsequent Primary Attacks", ConCommandFlag::AdminOnly);
	@weapon_knife_damage_charg = CCVar("weapon_knife_damage_charg", 30, "Knife damage on Secondary Attack", ConCommandFlag::AdminOnly);
	@weapon_knife_damage_throw = CCVar("weapon_knife_damage_throw", 20, "Knife damage on Tertiary Attack", ConCommandFlag::AdminOnly);
	...
	// Un-comment if you are using metamod plugin by Gaftherman -Mikk
...
{
	if( pMonster !is null
	//and g_CustomEntityFuncs.IsCustomEntity( 'weapon_knife' )
	and pMonster.GetClassname().StartsWith( 'monster_human_grun' ) )
	{
		CBaseEntity@ pKnife = g_EntityFuncs.CreateEntity( 'weapon_knife', null, true );

		if( pKnife !is null )
		{
			g_EntityFuncs.SetOrigin( pKnife, pMonster.pev.origin );
			pKnife.pev.spawnflags |= 1024;
		}
	}
